import 'package:flutter/material.dart';


class Info extends StatefulWidget {
  @override
  _InfoState createState() => _InfoState();
  
}

class _InfoState extends State<Info> {
  @override
  Widget build(BuildContext context) {
    
    return MaterialApp(
        debugShowCheckedModeBanner: false,
        theme: ThemeData(primaryColor: Color(0xff99e8e0)),
        home: Scaffold(
          appBar: AppBar(title: Center(child: Text('Ecom App UI'))),
          body: Container(
            width: 300,
            child: Stack(children: <Widget>[
               Stack(
            children: <Widget>[
              Image(
                width: 100,
                height: 100,
                image: NetworkImage('data:image/png;base64,'),
              ),
              Container(
                margin:EdgeInsets.only(left:150),
                height: 100,
                  child: Column(children: [
                Text('User'),
                SizedBox(height: 2),
                Text('abc@gmail.com'),
                SizedBox(height: 5),
                Text('logout'),
              ]

              
              )
              
              )]),
              Container(
                 margin:EdgeInsets.only(top:100) ,
                  child: Column(children: [
                Center(child: Text('Account Information', style:TextStyle(fontWeight: FontWeight.bold),)),
              
              ])
              ),
   

 Container(child :  Text('Email', style:TextStyle(fontWeight: FontWeight.bold),), padding: EdgeInsets.only(top: 120),),

 Container(child: Text('Phone',style:TextStyle(fontWeight: FontWeight.bold),),padding: EdgeInsets.only(top: 140),),

 Container(child: Text('Address',style:TextStyle(fontWeight: FontWeight.bold),),padding: EdgeInsets.only(top: 160),),
 
 Container(child: Text('Gender',style:TextStyle(fontWeight: FontWeight.bold),), padding: EdgeInsets.only(top: 180),),
            
            ],
            
            ),
          

             
           ),
            
    )
    );
         
  }
}